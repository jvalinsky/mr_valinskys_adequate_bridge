...
func (h *clientUIHandler) Mount(r chi.Router) {
	...
	r.Post("/replication", h.handleReplicationAction)
	r.Get("/events", h.handleEvents)
	r.Get("/settings", h.handleSettings)
	r.Post("/settings/export", h.handleSettingsExport)
	r.Post("/settings/import", h.handleSettingsImport)
	r.Get("/settings/diagnostics", h.handleSettingsDiagnostics)
	...
	r.Get("/api/capabilities", h.handleAPICapabilities)
	r.Get("/api/timeline", h.handleAPITimeline)
	r.Get("/api/thread/{msgKey}", h.handleAPIThread)
	r.Get("/api/channels", h.handleAPIChannels)
	r.Get("/api/votes", h.handleAPIVotes)
	r.Get("/api/search", h.handleAPISearch)
	r.Get("/api/followers", h.handleAPIFollowers)
	r.Get("/api/blob/{hash}", h.handleBlobsGet)
	r.Get("/api/conversations", h.handleAPIConversationsV2)
	r.Get("/api/conversations/{id}", h.handleAPIConversationV2)
	r.Post("/api/messages/send", h.handleAPIMessagesSend)
	r.Get("/api/room/state", h.handleAPIRoomState)
	r.Get("/api/room/invites", h.handleAPIRoomInvites)
	r.Post("/api/room/invites", h.handleAPIRoomInvites)
